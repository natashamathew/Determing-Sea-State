...
for i = 1:nvi % For each Valid Image
 ...
 for j = 1:6 % For each subimage (of 6)
     ...
     for k = 1:length(lines)
        ...
        bflr = zeros(1,480); % create an array of all zero 
        ...
        [mK2,imK2] = min(K2);
        hold on
        plot(1:480,imK2,'m.','LineWidth',2) % connect points and draw magneta line 
        pc = polyfit(1:480,imK2,1);
        for j = 1:480
            bflr(j) = pc(1)*j+pc(2);% pc(1) is slope & pc(2) is the intercept 
